...
const ModalDeleteDetailRental = ({ setParentDeleteRentalDetail, data }) => {
  const [deleteLoading, setDeleteLoading] = useState(false);
  const deleteDetailRental = async () => {
    setDeleteLoading(true);
    try {
      await axios
        .post("http://localhost:3000/api/admin/rentals/deleteDetailRental", {
          id: data.id,
        })
        .then((res) => {
          if (res.status == 200) {
            if (res.data.msg == "success") {
              setParentDeleteRentalDetail(false, true, {
                type: "success",
          ...
          <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
            ...
            <div className="flex items-center justify-end p-6 border-t border-solid border-blueGray-200 rounded-b">
              <button
                ...
                onClick={() => setParentDeleteRentalDetail(false)}
              >
                Batal
              </button>
              <button
...
export default ModalDeleteDetailRental;
